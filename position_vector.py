...
class PositionVector(object):
    ...
    def set_from_location(self, location):
        ...
        self.y = (location.lon - posvec_home_location.lon) * \
            posvec_latlon_to_m * posvec_lon_scale
        self.z = location.alt
    ...
    def get_bearing(cls, origin, destination):
        ...
        bearing = math.radians(
            90) + math.atan2(-(destination.x-origin.x), destination.y-origin.y)
        if bearing < 0:
    ...
    def main(self):
        ...
        PositionVector.set_home_location(
            LocationGlobalRelative(-35.362938, 149.165085, 0))
        print ("Home %s" % PositionVector.get_home_location())
        home_pos = PositionVector(0, 0, 0)
        print ("Home %s" % home_pos)

        # other position
        other_pos = PositionVector.get_from_location(
            PositionVector.get_home_location())
