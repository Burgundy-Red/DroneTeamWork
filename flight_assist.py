...
# 秦健实现，
def goto_position_target_vector_frd(vehicle, forward, righe, down):
    """	
    Send SET_POSITION_TARGET_LOCAL_NED command to request the vehicle fly to a specified 
    location in the North, East, Down frame.
...
def goto(vehicle, dNorth, dEast):
    ...
    while vehicle.mode.name=="GUIDED": #Stop action if we are no longer in guided mode.
        ...
        if remainingDistance<=2.5: #Just below target, in case of undershoot.
            print ("Reached target")
            break
    time.sleep(1)

def goto_point(vehicle, targetLocation, speed):
    '''
    Moves the vehicle to a targetLocation.
    The method takes a function pointer argument with a single 
    `dronekit.lib.LocationGlobal` parameter for the target position.
    This allows it to be called with different position-setting commands.
    By default it uses the standard method: dronekit.lib.Vehicle.simple_goto().

    Args:
        vehicle:
        targetLocation(vehicle.LocationGlobalRelative):
        speed(float):
    Returns:
    '''
    currentLocation = vehicle.location.global_relative_frame
    targetDistance = get_distance_metres(currentLocation, targetLocation)
    gotoFunction(targetLocation, speed)
    
    logger.debug(f"Go to point {targetLocation}")
    logger.debug(f"Target distance {targetDistance}")
    ...
    while vehicle.mode.name == "GUIDED":         
        ...
        #print ("Distance to target: ", remainingDistance)
        # if remainingDistance<=targetDistance*0.01: #Just below target, in case of undershoot.
        if remainingDistance <= 10: #Just below target, in case of undershoot.
            print ("Reached target")
            break
    time.sleep(1)
...
# 相对于无人机自身的速度进行控制 https://blog.csdn.net/liberatetheus/article/details/78005111
...
def send_global_velocity(vehicle, velocity_x, velocity_y, velocity_z, duration):
    ...
    for x in range(0, duration):
        ...
        time.sleep(5)
        point_cur = vehicle.location.global_frame
        distance = get_distance_metres(point_cur, point_prev)
        time_elapsed += 5
...
def set_vehicle_velocity_yaw(vehicle, yaw, speed=5, duration=1):
    """
    根据 yaw 角，调用 send_ned_velocity 函数设置速度
    Args:
        vehicle:
        yaw(float): 
        speed(int): 
    ...
    Returns:
    """
    assert speed > 0 and isinstance(duration, int) and duration >= 1

    velocity_x = speed * 1.0 * math.cos(math.radians(yaw))
    velocity_y = speed * 1.0 * math.sin(math.radians(yaw))
    velocity_z = 0 # 向下
    send_ned_velocity(vehicle, velocity_x, velocity_y, velocity_z, duration)


def add_velocity_message_listener(vehicle, func=None):
    """
    监听的信息初始化
    Args:
        vehicle:
    ...
    """
    if func == None:
        func = lambda self, name, msg: print(f"{name}: {msg}")
    
    vehicle.add_message_listener('GLOBAL_POSITION_INT', func)


def add_attributes_listener_init(vehicle, attributes, func=None):
    """
    监听的属性初始化
    Args:
        vehicle:
    ...
    for attr in attributes:
        vehicle.add_attribute_listener(attr, func)
    

def constant_speed_flight_to(vehicle, direction='w', direction_command_set=['k', 'j', 'h', 'l', 'n', 'm']):
    """
    匀速向 direction 飞行
    Args:
        vehicle:
        direction: 方向
        direction_command_set: 指令集
    Returns:
    """
    # TODO 是否需要保持指令
    if direction == 'w':
    ...
    ind = direction_command_set.index(direction) 
    if 0 <= ind <= 1: # 上下
        velocity_z = 0.5 * (-1 if ind % 2 == 0 else 1)
    elif 2 <= ind <= 3: # 左右
        velocity_y = 0.5 * (-1 if ind % 2 == 0 else 1)
    elif 4 <= ind <= 5: # 前后
        velocity_x = 0.5 * (1 if ind % 2 == 0 else -1)
